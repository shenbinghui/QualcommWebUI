...
function(_, $, ko, config, service, chosen) {
	...
	function dealPhoneBooks(){
		...
		for(var i = 0; i < config.listMsgs.length; i++){
			...
			for(var j = 0; j < config.phonebook.length; j++){
				var book = config.phonebook[j];
                var last8Num = getLastNumber(book.pbm_number, config.SMS_MATCH_LENGTH);
				/*if($.inArray(getLastNumber(book.pbm_number, config.SMS_MATCH_LENGTH), tmp) == -1){
				 options.push(new Option(book.pbm_name + "/" + book.pbm_number , book.pbm_number, false, true));
				 tmp.push(getLastNumber(book.pbm_number, config.SMS_MATCH_LENGTH));
				 }*/
		...
		select.append(opts);
		select.chosen({max_selected_options: 5, search_contains: true, width: '580px'});
		showSmsListData();
	...
	checkDeleteBtnStatus = function(){
		...
		if(size == 0){
			...
			$("#smslist-delete").removeClass('disable_btn');
        	$("#smslist-delete").addClass('disable_btn');
		} else {
...
*/
		...
		$("#submit_quickSaveContactForm").die().live("click", function(){
			$("#quickSaveContactForm").submit();
		});
	};
	...
	getShowNameByNumber = function(num){
		for(var i = 0 ; i < config.phonebook.length; i++){
			if(getLastNumber(config.phonebook[i].pbm_number, config.SMS_MATCH_LENGTH) == getLastNumber(num, config.SMS_MATCH_LENGTH)){
