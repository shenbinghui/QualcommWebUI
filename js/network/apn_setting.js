	...
	function getPdpTypes(){
		...
		if(config.IPV6_SUPPORT){
            ...
            if (config.IPV4V6_SUPPORT) {
                pdpTypes.push(new Option('IPv4v6', 'IPv4v6'));
            }
...
//					self.profileChangeHandler();
					if(config.connect_flag == true){service.scanRegist();}else{config.connect_flag = false;}
                } else {
                    errorOverlay();
                }
            }, function(data) {
		...
		function addNewApn(){
			...
			service.addOrEditApn({
				...
				index: optionLen,//getFirstBlankApnIndex(),
		...
		self.submit_apn_setting_form = function() {
            $("#apn_setting_form").submit();
         };
