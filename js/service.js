define([ 'underscore', 'jquery', 'config/config'], function(_, $, config) {
    ...
    function getWifiBasic() {
        ...
        function prepare(params, isPost) {
            ...
            requestParams.cmd = "m_ssid_enable,RadioOff,NoForwarding,m_NoForwarding," + wpapsk + "MAX_Station_num,"+
                "SSID1,AuthMode,HideSSID,MAX_Access_num,EncrypType," +
                "m_SSID,m_AuthMode,m_HideSSID,m_MAX_Access_num,m_EncrypType,rotationFlag,wifi_sta_connection,show_qrcode_flag,m_show_qrcode_flag";
    ...
    function getCurrentlyAttachedDevicesInfo(){
        ...
        function deal(data) {
            ...
            var ssid1AttachedNum = 0;
            var ssid2AttachedNum = 0;
            for(var i = 0; attachedDevices && i < attachedDevices.length; i++ ){
                ...
                obj.ssid_index = attachedDevices[i].ssid_index ? attachedDevices[i].ssid_index : "1";
                if(obj.ssid_index == "1"){
                    ssid1AttachedNum++;
                }else if(obj.ssid_index == "2"){
                    ssid2AttachedNum++;
                }
                deviceArr.push(obj);
            }
            return {
                ...
                ssid1AttachedNum: ssid1AttachedNum, 
                ssid2AttachedNum: ssid2AttachedNum				
            };
        }
    }
    ...
    function getStatusInfo(){
        if (timerInfo.isLoggedIn === undefined) {
            ...
            return {
				...
				deviceStatus: timerInfo.deviceStatus
            };
        }
	 ...
	 */
    var timerQueryString = [ "modem_main_state", "pin_status","opms_wan_mode","loginfo","new_version_state","current_upgrade_state","network_provider","is_mandatory"];
    var loginTimerQueryString = ["signalbar","network_type", 
    ...
    'ssid', 'wifi_enable', 'dial_mode','device_decryption_state'];
    if(config.HAS_SMS){
...
//        }
		...
		timerInfo.opms_wan_mode = data.opms_wan_mode ? data.opms_wan_mode : "PPP";
		// TODO OTA
		...
		timerInfo.deviceStatus = data.device_decryption_state == "1" ? "1" : "0";
    }
    ...
    function getLoginStatus() {
    	if(timerInfo.isLoggedIn == true){    		
    		document.getElementById("login").style.display="none";
			document.getElementById("login_username").style.display="";
    	}
        if(timerInfo.isLoggedIn != undefined){
...
//                currentLoginStatus = undefined;
    ...
    function getWpsInfo() {
        ...
        function prepare(params, isPost) {
            var requestParams = {};
            requestParams.isTest = isTest;
            requestParams.cmd = "wifi_wps_index,WscModeOption,AuthMode,RadioOff,EncrypType,wps_mode,WPS_SSID,m_ssid_enable,SSID1,m_SSID,m_EncrypType,m_AuthMode,wifi_sta_connection";
            requestParams.multi_data = 1;
            return requestParams;
        }
        ...
        function deal(data) {
            if (data) {
                var result = {
                    ...
                    MAX_Station_num: $.isNumeric(data.MAX_Station_num) ? (data.MAX_Station_num+2) : (config.MAX_STATION_NUMBER+2),
                    MAX_Access_num: data.MAX_Access_num,
                    m_MAX_Access_num: data.m_MAX_Access_num,
					ap_station_enable:data.wifi_sta_connection
                };
    ...
    function getDeviceInfo(){
        ...
        function prepare(params, isPost) {
            var requestParams = {
            	isTest: isTest,
                cmd : "wifi_coverage,m_ssid_enable,imei,network_type,rssi,rscp,lte_rsrp,imsi,sim_imsi,cr_version,wa_version,hardware_version,web_version,wa_inner_version,MAX_Access_num," +
                		"SSID1,AuthMode,WPAPSK1_encode,m_SSID,m_AuthMode,m_HideSSID,m_WPAPSK1_encode,m_MAX_Access_num,lan_ipaddr," +
                		"mac_address,msisdn,LocalDomain,wan_ipaddr,static_wan_ipaddr,ipv6_wan_ipaddr,ipv6_pdp_type,pdp_type,opms_wan_mode,ppp_status,sta_ip_status,device_decryption_state",
                multi_data : 1
            };
            return requestParams;
        }
        ...
        function deal(data) {
            if (data) {
                return {
                	...
                	imei: data.imei,
					signal: config.PRODUCT_TYPE == "CPE"? data.rssi : convertSignal(data),
					...
					opms_wan_mode:data.opms_wan_mode ? data.opms_wan_mode : "PPP",
					connectStatus: data.ppp_status,
					wifiConStatus: data.sta_ip_status,
					deviceStatus: data.device_decryption_state == "1" ? "1" : "0"
                };
            } else {
                return unknownErrorObject;
            }
	...
	function getOpMode(){
        ...
        function deal(data) {
            if (data) {
                return {
                    opms_wan_mode: config.PRODUCT_TYPE == 'CPE' || config.RJ45_SUPPORT ? data.opms_wan_mode : 'PPP',
    ...
    function  getNewVersionState() {
        ...
        function prepare(params, isPost) {
            var requestParams = {};
            requestParams.isTest = isTest;
            requestParams.cmd = "new_version_state,fota_package_already_download";
			requestParams.multi_data = 1;
            return requestParams;
        }
...
/**
    ...
    function getCellId(){
        ...
        function prepare(params, isPost) {
            var requestParams = {};
            requestParams.isTest = isTest;
            requestParams.cmd = "cell_id_list,global_cell_id,network_type,cell_not_correct";
            requestParams.multi_data = 1;
            return requestParams;
        }
