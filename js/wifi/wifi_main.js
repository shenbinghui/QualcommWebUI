...
define([ 'jquery', 'knockout', 'config/config', 'service', 'underscore' ],
    ...
    function ($, ko, config, service, _) {
        ...
        function WifiBasicVM() {
			...
			self.showQRSwitch = config.WIFI_SUPPORT_QR_SWITCH;
            ...
            self.checkSettings = function (ssid) {
               ...
               /* if (!config.WIFI_SWITCH_SUPPORT && status.radioFlag == "0") {
                ...
                if(ssid == "ssid1" || ssid == "ssid2"){					
                    if(ssid == "ssid1"){
                        var accessDevice = service.getCurrentlyAttachedDevicesInfo().ssid1AttachedNum;
                        if(parseInt(self.selectedStation()) < accessDevice){
                            showAlert('Extend_accessDevice');
                            return true;
                        }
                    } else{
                        var accessDevice = service.getCurrentlyAttachedDevicesInfo().ssid2AttachedNum;
